...
bool TransactionRepository::insertTransaction(const Transaction& transaction) {
    ...
    if (!success) cerr << "Failed to execute INSERT: " << sqlite3_errmsg(db) << endl;
...
bool TransactionRepository::updateTransaction(const Transaction& transaction) {
    ...
    bool success = (sqlite3_step(stmt) == SQLITE_DONE);
    if (!success) cerr << "Failed to execute UPDATE: " << sqlite3_errmsg(db) << endl;
...
bool TransactionRepository::deleteTransaction(int transactionId) {
    ...
    bool success = (sqlite3_step(stmt) == SQLITE_DONE);
    if (!success) cerr << "Failed to execute DELETE: " << sqlite3_errmsg(db) << endl;
...
// ------------------- Find by ID -------------------
Transaction* TransactionRepository::getById(int transactionId) {
    const char* sql =
